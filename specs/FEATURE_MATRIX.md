...
| Focus management with roving tabindex and restoration | Partial | WorkspaceExplorer/WorkspaceEditor roving tabindex; CommandPalette list selection; a reusable useFocusRestore hook (a module-level non-dialog focus tracker) returns focus to the pre-modal element when the command palette and the theme export/import and notification modals close (two e2es cover the palette and the theme-export modal). | No reusable focus-trap hook or per-mode focus order. |
