...
| Layout persistence across sessions | Partial | useWorkspace.ts persists root paths and recents; App.tsx restores presentationMode and the browser demo's active file (openFile, persisted to localStorage and validated against the demo files by parseActiveFile, unit tested; an e2e reopens it after a reload). | No split sizes, panel positions, or multi-tab order persisted, and the native workspace's active tab is not restored on reopen. |
