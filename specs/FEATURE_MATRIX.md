...
| Layout persistence across sessions | Partial | useWorkspace.ts persists root paths and recents; App.tsx restores presentationMode, the browser demo's active file (openFile, validated against the demo files by parseActiveFile), and the primary sidebar visibility and open bottom panel (sidebarHidden and bottomPanel, validated by parseBottomPanel) across a reload. parseActiveFile and parseBottomPanel are unit tested; e2es cover the active-file and the sidebar/panel reloads. | No editor split sizes, dockable panel positions, or multi-tab order persisted, and the native workspace's active tab is not restored on reopen. |
