...
| 5.16 | Themes and appearance | 19 | 1 | 3 | 0 |
...
| Column/box selection | Done | editor.columnSelection is now a setting mapped to Monaco (editorSettingsToMonaco, unit tested) and toggleable in the Settings UI, making the column-selection mode explicit config rather than an implicit default (default false matches Monaco, so no behavior change); box selection via Shift+Alt+drag was already a Monaco standalone default. | |
...
| EOL handling (LF/CRLF detect, convert, platform default) | Partial | FilePayload reports eol; the status bar shows the model EOL and its item opens a Change End of Line picker that converts the line endings live (setEditorEol via the editor bridge, model.pushEOL); unit and e2e covered. A Default End of Line setting (auto/LF/CRLF; resolveDefaultEol pure, unit tested) sets the line ending of new untitled scratchpads, with auto following the OS (CRLF on Windows, LF elsewhere); an e2e forces LF, creates an untitled, and asserts the status-bar EOL (it fails without the wiring). | The default EOL applies only to untitled scratchpads; explorer/palette-created workspace files (ws.newFile, native) still use Monaco's platform default. |
...
| Read-only editors and read-only regions | Partial | A View: Toggle Editor Read-only command toggles a session read-only state that EditorPanel applies as Monaco's readOnly option (data-readonly on the panel; an e2e confirms edits are rejected while on and accepted after toggling off). Diff tabs are also read-only. | No per-file readonly flag from fs or IPC (the toggle is a manual session state, not file-sourced), and no read-only regions. |
...
| Inlay hints | Partial | The TS and JS workers provide parameter-name and type hints via setInlayHintsOptions (TS_INLAY_HINTS, unit tested), and rendering is toggled by the Inlay Hints editor setting (editor.inlayHints, default on, mapped to Monaco inlayHints.enabled on/off; unit tested, control covered by an e2e). | Hints come from the worker only; no custom InlayHintsProvider for other languages. |
...
| Markdown preview (synced scroll, link completion, math/diagram, broken-link diagnostics) | Partial | renderMarkdown (lib/markdown.ts) is a safe Markdown to HTML renderer (escapes HTML, sanitizes hrefs) shown by MarkdownPreview via the Markdown: Open Preview command, thoroughly unit tested. Broken-link diagnostics: findBrokenLinks (lib/markdown-links.ts, unit tested) flags inline-link targets that do not resolve to a known workspace file (resolving relative paths; skipping external and anchor targets, and links inside code spans or fenced blocks), surfaced as Problems-panel warnings via Monaco markers; an e2e covers it. Link-path completion: completeMarkdownLinkPath (unit tested) suggests workspace files while typing a [text](... target, wired as a Monaco completion provider fed the workspace file list; an e2e covers it. | No synced-scroll or math or diagram rendering. |

## 5.3 Editor advanced surface

The editor uses Monaco 0.55.1. Editor appearance and behavior are now exposed through the Settings panel (font, minimap, cursor, scrolling, rulers, indent guides, and bracket colorization), and the Outline view sources symbols for Markdown, JSON, CSS, YAML, TOML, Python, and JS/TS/Rust code files. A few advanced surfaces (sticky scroll, inline diagnostics from language workers) still rely on Monaco defaults that are on but lack dedicated UI controls. The app emphasizes simplicity in early alpha over full parity on the advanced surface.
...
| Outline rendering source for Outline view | Partial | OutlinePanel renders outlineSymbols (lib/workspace-symbols.ts, unit tested): Markdown headings, JSON top-level keys (jsonSymbols), top-level CSS selectors (cssSymbols, a string-aware brace-depth scan that skips comments, at-rule inner rules, and declarations, and ignores braces inside strings), top-level YAML mapping keys (yamlSymbols, column-0 keys; skips nested keys, list items, and comments), TOML tables and array-of-tables plus top-level keys (tomlSymbols, which skips keys under a table and the contents of multi-line strings), top-level Python def and class declarations (pythonSymbols, column-0; nested defs and triple-quoted-docstring contents are skipped), plus flat top-level declarations for JS/TS and Rust files via codeSymbols. Clicking an entry reveals its line; e2es cover Markdown, code, JSON, CSS, YAML, TOML, and Python. | No structural outline for SCSS/LESS (// line comments unhandled) or HTML; YAML quoted keys are not parsed; code coverage is flat top-level declarations only (JS/TS, Rust, and Python), with no nesting. |
...
| Compare feature (select, with saved, across workspace, folders) | Partial | Compare with Saved diffs the editor's unsaved changes against disk; File: Select for Compare stashes the active file and File: Compare with Selected diffs a second file against it in the Monaco diff modal (both work in the workspace and the demo). An e2e covers select-then-compare. | No folder comparison, and no whole-workspace compare. |
...
| Primary and secondary sidebars (left/right) | Partial | App.tsx renders left explorer and right sidebar (companion, timeline); placement hardcoded per mode. The primary sidebar toggles with View: Toggle Primary Sidebar (Ctrl/Cmd+B), and the secondary (right) sidebar with View: Toggle Secondary Sidebar (per-mode grid override that lets the editor reclaim the space, persisted across reload); both covered by e2es. | Views cannot move between sidebars. |
| Bottom panel (terminal, problems, output, debug console) | Partial | App.tsx footer renders TerminalTabs, TokenPanel, Narration; only terminal is wired. The dock sash-resizes (the bottom Sash, slice 110) and maximizes to fill the workbench via View: Toggle Maximized Panel (data-bottom-maximized overrides --bottom-height; e2e covered). | No problems, output, or debug console as panel tabs; problems and output open as a separate bottom drawer, there is no debug console, and the panel cannot be moved. |
| Sash resizing (visual splitter) | Partial | The primary sidebar and the bottom panel dock both have draggable, keyboard-operable sashes (components/Sash.tsx, an ARIA separator with Arrow/Home/End in vertical and horizontal orientations) that resize them via --sidebar-width / --bottom-height CSS vars, clamped and persisted to localStorage. The pure math lives in lib/sash.ts (unit tested); two e2es resize each with the keyboard. | Editor groups are not draggable and there are no editor splits to size. |
...
| Zen mode (hide UI chrome) | Done | The View: Toggle Zen Mode command sets data-zen on the shell, hiding the header, activity bar, panels, and status bar to leave just the editor, which fills the full width even when a sidebar was hidden first (the grid collapses to one column; e2es cover both the primary and secondary cases); Escape exits. | |
...
| Layout persistence across sessions | Partial | useWorkspace.ts persists root paths and recents; App.tsx restores presentationMode, the browser demo's active file (openFile, validated against the demo files by parseActiveFile), and the primary and secondary sidebar visibility and open bottom panel (sidebarHidden, secondarySidebarHidden, and bottomPanel, the last validated by parseBottomPanel) across a reload. parseActiveFile and parseBottomPanel are unit tested; e2es cover the active-file, sidebar/panel, and secondary-sidebar reloads. | No editor split sizes, dockable panel positions, or multi-tab order persisted, and the native workspace's active tab is not restored on reopen. |
...
| Show/hide individual views independently | Partial | The primary sidebar toggles with Ctrl/Cmd+B and the bottom panel with Ctrl/Cmd+J (View: Toggle Panel, restoring the last panel); both layout toggles are e2e covered. The secondary (right) sidebar toggles with View: Toggle Secondary Sidebar. Problems, Search, and Source Control also toggle from the activity bar, and Output and Outline from their View commands. | The companion and timeline panels are not individually toggleable (only together, as the secondary sidebar), and the terminal footer is still mode-coupled. |
...
| Symbol navigation in current file with @ | Done | Typing @ in the palette lists the active file's outline symbols inline (outlineSymbols over markdown headings, JSON top-level keys, CSS selectors, YAML top-level keys, TOML tables, Python defs/classes, and top-level JS/TS/Rust declarations) and jumps to the chosen symbol's line; a trailing row hands off to the editor's full Go to Symbol (Monaco quickOutline) for nested symbols and other languages. parsePaletteInput symbols mode and outlineSymbols are unit tested; an e2e jumps to a symbol. | |
| Workspace symbols search with # | Partial | Typing # in the palette searches a workspace-symbol index (buildWorkspaceSymbols over the file contents: markdown headings and top-level code declarations via codeSymbols; filterWorkspaceSymbols; all unit tested); selecting opens the file and jumps to the symbol's line (openSymbol plus EditorPanel revealLine, race-free across the model swap). An e2e jumps cross-file to a symbol. | Built from the demo file contents only (a real workspace needs the core to supply file bodies). |
...
| Search history | Done | SearchPanel keeps a recent-query history (pushSearchHistory, unit tested); Up and Down in the search box recall previous queries. The history is persisted to localStorage (loadSearchHistory/saveSearchHistory; parseSearchHistory tolerates corrupt storage, unit tested) and survives a reload (an e2e searches, reloads, and recalls the query). | |
...
| Stash (create/apply/pop/drop/list/include-untracked) | Done | git.rs git_stash (push -u), git_stash_pop, git_stash_list, git_stash_apply, and git_stash_drop (cargo tested); the GitStashModal (Git: Stashes) lists the stashes (parseStashList, unit tested) with Apply and Drop and a Stash Current Changes button; the SCM panel keeps Stash and Pop. | No named stash. |
...
| Task groups (build, test) and default task | Partial | Tasks carry a build or test group (parseTasksJson reads the tasks.json group; detectNpmTasks classifies script names via classifyTaskGroup, unit tested); Tasks: Run Build Task runs the first build-group task and is bound to Ctrl or Cmd plus Shift plus B (the global key handler invokes the command; an e2e fires the chord). | No persisted default task. |
...
| Keybindings editor / settings UI | Partial | KeyboardShortcuts.tsx is a searchable reference of every command and its shortcut, opened by the Preferences: Keyboard Shortcuts command, and flags per-key conflicts. | Read-only: no rebinding or keybindings.json persistence. |
...
| Per-key conflict detection | Partial | keybinding-conflicts.ts findKeybindingConflicts normalizes gestures (case, modifier order, chords) and groups commands sharing one; the Keyboard Shortcuts view shows a conflict summary plus a per-row conflict badge. Unit tested. | Detection and warning only over registered commands; no live warning while rebinding (no rebinding yet) and no when-clause scoping. |
...
| Follow OS system theme | Done | The workbench.followSystemTheme setting makes App listen to matchMedia(prefers-color-scheme) and switch between the user's Preferred Dark Theme and Preferred Light Theme (both Appearance selects over the bundled themes, defaulting to cozy-dark/cozy-light) via themeForSystem (parametrized, unit tested). Changing a preference while following re-applies it. An e2e emulates an OS dark preference with a chosen preferred dark theme and asserts it applies (html data-theme); it fails if the preference is not wired through. | |
...
| Custom theme file import/export | Done | Theme: Export shows the active theme JSON (Copy). Theme: Import parses a pasted theme (parseImportedTheme, unit tested, requiring the full color token set so a partial paste errors instead of importing as a no-op) and applies it as a custom theme (themeId "custom" plus customTheme in settings; picking any built-in theme makes it inert without a clear). currentTheme/applyTheme honor it and exportTheme round-trips. Both unit tested; an e2e imports a theme and asserts it applies. | (Applied directly as CSS variables, not registered as a selectable named theme or contributed by an extension.) |
...
| Full keyboard operability with visible focus | Partial | focus-visible ring in styles.css; Ctrl/Cmd+K; explorer and tab-bar roving; context-menu escape; F6 and Shift+F6 cycle focus through the activity bar, sidebar, editor, panel, and status bar, skipping regions with no focusable element (e2e covers cycling, wrap-around, and the skip in minimal mode); keyboard pane resize via the sashes (Arrow/Home/End); modal focus trap via useFocusTrap (Tab stays within the command palette and the theme export/import and notification modals; e2e covered). | No Alt+Z/Alt+H or Ctrl/Cmd+. bindings. |
...
| Focus management with roving tabindex and restoration | Partial | WorkspaceExplorer/WorkspaceEditor roving tabindex; CommandPalette list selection; reusable useFocusRestore and useFocusTrap hooks return focus to the pre-modal element on close and trap Tab within the open modal (applied to the command palette and the theme export/import and notification modals; e2es cover restore on two modals and the Tab trap). | No per-mode focus order. |
...
The repository has a growing preview surface (an image viewer for raster and SVG files with dimensions, zoom, rotate, and drag-to-pan, and a read-only hex dump) on top of text-based Monaco editing with language-specific highlighting. Still absent are custom document models, backup systems, webview messaging, content security policies, and custom data contributions. The agent-edit and diff surface (spec section 11) is designed but the code shows only basic edit visualization with text diffs. Preview tabs exist in the spec but the implementation has basic tab management without preview semantics.
...
| Audio/video media player | Partial | Media: Open Player opens audio and video files (lib/media.ts isMediaPath, mediaKind, mediaMime, unit tested) in a MediaPlayer with native browser controls; an e2e covers a WAV. Natively the file's bytes are read as base64 (fs_read_file_base64, cargo tested) and wrapped in a data URL the CSP's media-src allows; the browser demo uses a stored data URL. | No playlist, captions, or playback-speed control. |
| Hex viewer and binary editor | Partial | View: Hex shows a hex dump of the active file (hexDumpBytes in lib/hex.ts, unit tested) in HexView. Bytes are now exact: natively the file's true on-disk bytes (fs_read_file_base64 plus base64ToBytes, so unsaved edits are not reflected), and in the demo the content encoded as UTF-8. An e2e covers it. | Read-only, no byte editing or endianness/data-inspector. |
