...
```mermaid
flowchart TD
    A[Report received] --> B[Acknowledge within 3 business days]
    B --> C[Review and investigate]
    C --> D{Violation found?}
    D -->|No| E[Explain decision to reporter]
    D -->|Yes| F[Choose action by impact level]
    F --> G[Communicate action to those involved]
    G --> H[Follow up and document outcome]
    E --> H
```
